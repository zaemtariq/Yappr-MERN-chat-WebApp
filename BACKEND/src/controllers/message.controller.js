...
export const getUsersForSideBar = async (req, res) => {
    try {
        const loggedInUsers = await User.findById(req.user._id);
        const filteredUsers = await User.find({ _id: { $ne: loggedInUsers } }).select("-password");
        res.status(200).json(filteredUsers);
    } catch (error) {
        console.log(error.message)
        res.status(200).json({ message: "Users for Side Bar Not Found!" });
    }
}
...
export const sendMessage = async (req, res) => {
    try {
        ...
        console.log(error)
        res.status(201).json({ message: "Message Sending Error!" })
    }
}